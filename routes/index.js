...
router.get('/api/layer/:dbname', function(req, res) {
    ...
    pg.connect(connectionString, function(err, client, done) {
        ...
        }else if(dbname = 'restaurant_buff_40_m'){
            var query = client.query("SELECT row_to_json(fc) FROM (SELECT 'FeatureCollection' As type, array_to_json(array_agg(f)) As features FROM (SELECT 'Feature' As type, ST_AsGeoJSON(lg.geom)::json As geometry, row_to_json((SELECT l FROM (SELECT gid) As l)) As properties FROM "+dbname+" As lg   ) As f )  As fc;");
        } else {
...
router.post('/api/layer', function(req, res) {
    ...
    pg.connect(connectionString, function(err, client, done) {
        ...
        var query = 'CREATE TABLE '+newdbname+'(gid SERIAL PRIMARY KEY, geom AS ST_AsText(ST_Buffer(db.geom, '+dist+')) AS geom FROM '+dbname+' AS db)';
